...
import { Plus, Edit, Power, PowerOff, Loader2, MoreVertical } from 'lucide-react';
...
export function TiposPagoList({
  ...
  if (error) {
    return <TableErrorState message={`Error al cargar métodos de pago: ${error}`} onRetry={onRefetch} />;
  }
  ...
  return (
    <>
      ...
      <div className="bg-white shadow rounded-lg">
        ...
        {loading ? (
          <TableLoadingState message="Cargando métodos de pago..." />
        ) : tipos.length === 0 ? (
          <TableEmptyState message="No hay métodos de pago registrados" />
        ) : (
